...
aoi2$lands_dens=1000000*(aoi$lands_rec/aoi$area)
...
lmt = lm.RStests(col.fit1, aoi.listw, test = c("LMerr", "LMlag"))
summary(lmt)
...
col.fit9 <- lagsarlm(lands_dens ~ 0 + (elev_mean + slope_mean + RainfallDaysmean):(cuenca), data = aoi2,listw = aoi.listw)
...
col.fit11 = errorsarlm(lands_dens ~ 0 + (elev_mean + slope_mean + RainfallDaysmean):(cuenca), data = aoi2,listw = aoi.listw)
...
col.fit14 <- sacsarlm(lands_dens ~ 0 + (elev_mean + slope_mean + RainfallDaysmean):(cuenca), data = aoi2,listw = aoi.listw, type="sacmixed") 
summary(col.fit14,Nagelkerke=T)

#regimes-SAC-NOMZH
col.fit15 <- sacsarlm(lands_dens ~ 0 + (elev_mean + slope_mean + RainfallDaysmean):(cuenca), data = aoi2,listw = aoi.listw, type="sac")
summary(col.fit15)
...
moran.mc(residuals(col.fit16),nsim = 999,listw = aoi.listw,alternative = "greater")
summary(impacts(col.fit16, listw = aoi.listw, R=500), zstats = TRUE)

#regimes-SLX-NOMZH
col.fit17 = lmSLX(lands_dens ~ 0 + (elev_mean + slope_mean + RainfallDaysmean):(cuenca), data = aoi2,listw = aoi.listw)
